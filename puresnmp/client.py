...
class Client:
    ...
    def get(self, ip: str, oid: str, port: int = DEFAULT_SNMP_PORT):
        """
        Executes a simple SNMP GET request and returns a pure
    ...
    def multiget(self, ip: str, oids: List[str],
                 port: int = DEFAULT_SNMP_PORT):
        """
        Executes an SNMP GET request with multiple OIDs and returns
        a list of pure Python objects.
        The order of the output items is the same order as the OIDs given
        as arguments.
    ...
    def multigetnext(self, ip: str, oids: List[str],
                     port: int = DEFAULT_SNMP_PORT):
        """
        Function to send a single multi-oid GETNEXT request.

        The request sends one packet to the remote host requesting the value of
        the OIDs following one or more given OIDs.
        ...
        Example::
            ...
            [
                VarBind(ObjectIdentifier(1, 2, 3, 0),
                        'non-functional example'),
                VarBind(ObjectIdentifier(1, 2, 4, 0),
                        'second value')
            ]
        """
    ...
    def walk(self, ip: str, oid: str, port: int = DEFAULT_SNMP_PORT):
        """
        Executes a sequence of SNMP GETNEXT requests and returns an generator
        over :py:class:`~puresnmp.pdu.VarBind` instances.

        The generator stops when hitting an OID which is *not* a sub-node of
        the given start OID or at the end of the tree (whichever comes first).
        ...
        Example::
            ...
            [VarBind(oid=ObjectIdentifier((1, 3, 6, 1, 2, 1, 3,
                                           1, 1, 1, 24, 1, 172, 17, 0, 1)),
                     value=24),
             VarBind(oid=ObjectIdentifier((1, 3, 6, 1, 2, 1, 3,
                                           1, 1, 2, 24, 1, 172, 17, 0, 1)),
                     value=b'\\x02B\\xef\\x14@\\xf5'),
             VarBind(oid=ObjectIdentifier((1, 3, 6, 1, 2, 1, 3,
                                           1, 1, 3, 24, 1, 172, 17, 0, 1)),
                     value=64, b'\\xac\\x11\\x00\\x01')]
        """

        return self.multiwalk(ip, [oid], port)

    def multiwalk(self, ip: str, oids: List[str],
                  port: int = DEFAULT_SNMP_PORT, fetcher=None):
        """
        Executes a sequence of SNMP GETNEXT requests and returns an generator
        over :py:class:`~puresnmp.pdu.VarBind` instances.
    ...
    def set(self, ip: str, oid: str, value: Type,
            port: int = DEFAULT_SNMP_PORT):
        """
        Executes a simple SNMP SET request. The result is returned as
        pure Python data structure. The value must be a subclass of
    ...
    def multiset(self, ip: str, mappings: List[Tuple[str, Type]],
        ...
        Executes an SNMP SET request on multiple OIDs. The result is returned
        as pure Python data structure.
...
...                                      ('2.3.4', OctetString(b'bar'))])
        ...
        if any([not isinstance(v, Type) for k, v in mappings]):
            raise TypeError('SNMP requires typing information. '
                            'The value for a "set" request must be an '
                            'instance of "Type"!')
    ...
    def bulkget(self, ip: str, scalar_oids: List[str],
                repeating_oids: List[str], max_list_size: int = 1,
                port: int = DEFAULT_SNMP_PORT):
        """
        Runs a "bulk" get operation and returns a :py:class:`~.BulkResult`
        instance. This contains both a mapping for the scalar variables
        (the "non-repeaters") and an OrderedDict instance containing
        ...
        values ("repeating values") in one single request. You can for example
        retrieve the hostname (a scalar value), the list of interfaces
        (a repeating value) and the list of physical entities
        (another repeating value) in one single request.

        Note that this behaves like a **getnext** request for scalar values!
        So you will receive the value of the OID which is
        *immediately following* the OID you specified for both scalar and
        repeating values!
            ...
            BulkResult(
                scalars={'1.3.6.1.2.1.1.2.0': '1.3.6.1.4.1.8072.3.2.10',
                         '1.3.6.1.2.1.1.1.0': b'Linux aafa4dce0ad4 4.4.0-28-'
                                              b'generic #47-Ubuntu SMP Fri Jun'
                                              b' 24 10:09:13 UTC 2016 x86_64'},
                listing=OrderedDict([
        ...
        packet = Sequence(
            Integer(Version.V2C),
            OctetString(self._community),
            BulkGetRequest(self._get_request_id(), non_repeaters,
                           max_list_size, *oids)
        )
    ...
    def _bulkwalk_fetcher(self, bulk_size: int = 10):
        ...
        def fun(ip: str, oids: List[str],
                port: int = Client.DEFAULT_SNMP_PORT):
            result = self.bulkget(ip, [], oids, max_list_size=bulk_size,
                                  port=port)
    ...
    def bulkwalk(self, ip: str, oids: List[str], bulk_size: int = 10,
                 port: int = DEFAULT_SNMP_PORT):
        """
        ...
        :param bulk_size: How many varbinds to request from the remote host
            with one request.
        ...
        Example::
            ...
            >>> for row in result:
            ...     print(row)
            VarBind(oid=ObjectIdentifier((1, 3, 6, 1, 2, 1, 2, 2, 1, 2, 1)),
                    value=b'lo')
            VarBind(oid=ObjectIdentifier((1, 3, 6, 1, 2, 1, 2, 2, 1, 6, 1)),
                    value=b'')
            VarBind(oid=ObjectIdentifier((1, 3, 6, 1, 2, 1, 2, 2, 1, 22, 1)),
                    value='0.0')
            VarBind(oid=ObjectIdentifier((1, 3, 6, 1, 2, 1, 2, 2, 1, 2, 38)),
                    value=b'eth0')
            VarBind(oid=ObjectIdentifier((1, 3, 6, 1, 2, 1, 2, 2, 1, 6, 38)),
                    value=b'\x02B\xac\x11\x00\x02')
            VarBind(oid=ObjectIdentifier((1, 3, 6, 1, 2, 1, 2, 2, 1, 22, 38)),
                    value='0.0')
        """
    ...
    def table(self, ip: str, oid: str, port: int = DEFAULT_SNMP_PORT,
        ...
        *num_base_nodes* to 4).
        ...
        Example::
            ...
            >>> pprint(wlk)
            [VarBind(oid=ObjectIdentifier((1, 2, 3, 4, 1, 1, 192, 168, 0, 2)),
                     value=Integer(12)),
             VarBind(oid=ObjectIdentifier((1, 2, 3, 4, 1, 1, 192, 168, 0, 3)),
                     value=Integer(13)),
             VarBind(oid=ObjectIdentifier((1, 2, 3, 4, 1, 2, 192, 168, 0, 2)),
                     value=Integer(22)),
             VarBind(oid=ObjectIdentifier((1, 2, 3, 4, 1, 2, 192, 168, 0, 3)),
                     value=Integer(23)),
             VarBind(oid=ObjectIdentifier((1, 2, 3, 4, 1, 3, 192, 168, 0, 2)),
                     value=Integer(32)),
            ...
            [{'0': '192.168.0.2', '1': Integer(12), '2': Integer(22),
                                  '3': Integer(32)},
             {'0': '192.168.0.3', '1': Integer(13), '2': Integer(23),
                                  '3': Integer(33)}]
    ...
    def table_row_count(self, ip: str, oid: str, port: int = DEFAULT_SNMP_PORT,
                        num_base_nodes: int = 0) -> int:
        return len(self.table(ip, oid, port, num_base_nodes))

    def table_add_row(self, ip: str, table_oid: str, row_identifier: str,
                      row_status_column: int, fields: List[Tuple[int, Type]],
                      port: int = DEFAULT_SNMP_PORT):
        """
        The *row_identifier* (or Instance-Identifier per the RFC2579) can be a
        simple number that designate the row (like '1' for the first row, '2'
        for the second, etc.) or be more complexe (like '192.168.0.1' for the
        first row, '192.168.0.2' for the second, etc.)

        The *row_status_column* is the index of the column where the RowStatus
        information can be found.

        The *fields* is a list tuple where the first element is the index of a
        column and the second is the value of the column to set.

        The row is added using the 'createAndGo' method, creating the row in
        only one transaction, instead of the 'createAndWait' method which use
        multiple, coordinated set requests.
        ...
        Example::
            ...
            ... ]
            >>> c.table_add_row('::1', '1.2.3.4.1', row_identifier='3',
            ...                 row_status_column=10, fields=fields)

        """

        # Specs:
        #   - RFC 2579 Page 10-16: https://tools.ietf.org/html/rfc2579
        #   - Webnms docs: https://www.webnms.com/snmp/help/snmpapi/snmpv3/
        #                   table_handling/snmptables_addrow.html
        ...
        # We only GET on the column rowStatus to check if the *row_identifier*
        #   already exist.
        row_status_oid = (table_oid + '.' + str(row_status_column)
                          + '.' + row_identifier)
        try:
            row_status = self.get(ip, row_status_oid, port)
            raise SnmpError("An other management station has already created "
                            "this row. Identifier: {}".format(row_identifier))
        except SnmpError:
            # Todo: We should check the SNMP exception !
            # (noSuchInstance: Ok or noSuchObject: not OK)
            pass
        ...
        for column_idx, value in fields:
            if column_idx == row_status_column:
                raise ValueError('The value of the column rowStatus must be '
                                 'handled by table_add_row()')
        ...
        if row_status != RowStatus.ACTIVE:
            raise SnmpError("Could not add the row, rowStatus: {}"
                            .format(row_status))

        return output

    def table_delete_row(self, ip: str, table_oid: str, row_identifier: str,
                         row_status_column: int,
                         port: int = DEFAULT_SNMP_PORT):
